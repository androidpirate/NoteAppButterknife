package com.example.android.noteappbutterknife.db;
...
public class NoteDbHelper extends SQLiteOpenHelper {
    ...
    private static final String SQL_CREATE_TABLE = "CREATE TABLE " + NoteContract.NoteEntry.TABLE_NAME + "(" +
            NoteContract.NoteEntry._ID + " INTEGER PRIMARY KEY AUTOINCREMENT" + COMMA_SEPARATOR +
            NoteContract.NoteEntry.NOTE_TITLE + " TEXT NOT NULL" + COMMA_SEPARATOR +
            NoteContract.NoteEntry.NOTE_DESCRIPTION + " TEXT NOT NULL " + ");";
    // SQL command to drop a table
    private static final String SQL_DROP_TABLE = "DROP TABLE IF EXISTS " + NoteContract.NoteEntry.TABLE_NAME;
    // Static instance
    ...
    public List<Note> getAllNotes() {
        ...
        try {
            if(cursor.moveToFirst()) {
                do {
                    Note note = new Note();
                    note.set_id(cursor.getInt(cursor.getColumnIndex(NoteContract.NoteEntry._ID)));
                    note.setTitle(cursor.getString(cursor.getColumnIndex(NoteContract.NoteEntry.NOTE_TITLE)));
                    note.setDescription(cursor.getString(cursor.getColumnIndex(NoteContract.NoteEntry.NOTE_DESCRIPTION)));
                    notes.add(note);
    ...
    public void insertNote(Note note) {
        ...
        try {
            ...
            values.put(NoteContract.NoteEntry.NOTE_DESCRIPTION, note.getDescription());
             // No id is needed to insert a note to database since
             // SQLite automatically increments the id numbers
            db.insertOrThrow(NoteContract.NoteEntry.TABLE_NAME, null, values);
